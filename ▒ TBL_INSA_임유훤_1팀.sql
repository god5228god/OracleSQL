...
COUNTRIES1	TABLE
COUNTRIES	TABLE
BIN$z6rE1yh7RWmgwqBpOPvb9g==$0	TABLE
BIN$rSSXF3W8TfCwehqIQ3SZzA==$0	TABLE
BIN$nWKbanvNRgah/1J00+IO6Q==$0	TABLE
BIN$g8BsUE8FSSCfOAu4a8afyg==$0	TABLE
BIN$fSL9xLF1Q+Sb/zeS9KTFng==$0	TABLE
BIN$bdxuU0VCTOy7KAAYpx1Y9g==$0	TABLE
BIN$VnuWfFwVQE6gRMZnHPpArA==$0	TABLE
BIN$Tz0SZnFvTouRRmqCiPZ8FQ==$0	TABLE
BIN$TP67KK4gQrmhLEoghZPWdg==$0	TABLE
BIN$R9nDDiCtTXCW5Hl5hy0EIw==$0	TABLE
BIN$R1hLotNDQySfT53BAYQZog==$0	TABLE
BIN$P3QPhJB/Sh+eOY5mIB7A6A==$0	TABLE
BIN$MrGmqy8rS0OPjL8uLOTqRA==$0	TABLE
BIN$JZCu+BJ7Q2eZq0Llz7M+JQ==$0	TABLE
BIN$By9tTUebRqivJl8D4QNlaA==$0	TABLE
*/
SELECT TABLE_NAME
FROM USER_TABLES;
--==>>
...
FROM TBL_INSA
WHERE CITY ='인천' AND
    (BASICPAY >= 1000000 AND BASICPAY < 2000000 );
--==>>
/*
1014	황진이	810707-2574812	2002-02-15	인천	010-3214-5467	개발부	사원	1100000	130000
1035	정영희	831010-2153252	2002-05-16	인천		            개발부	사원	1050000	140000
...
SELECT NAME, SSN
FROM TBL_INSA
WHERE SUBSTR(SSN,1,2) BETWEEN '80' AND '89';
--==>>
/*
이순신	801007-1544236
한석봉	811112-1566789
...
FROM TBL_INSA
WHERE CITY = '서울' AND
    SUBSTR(SSN,1,2) BETWEEN '70' AND '79';
--==>>
/*
1001	홍길동	771212-1022432	1998-10-11	서울	011-2356-4528	기획부	부장	2610000	200000
1019	박문수	780710-1985632	1999-12-10	서울	017-4747-4848	인사부	과장	2300000	165000
...
SELECT *
FROM TBL_INSA
WHERE ( CASE WHEN SUBSTR(SSN,8,1) IN ('1','2') THEN EXTRACT(YEAR FROM SYSDATE) - TO_NUMBER('19'||SUBSTR(SSN,1,2)) + 1
             WHEN SUBSTR(SSN,8,1) IN ('3','4') THEN EXTRACT(YEAR FROM SYSDATE) - TO_NUMBER('20'||SUBSTR(SSN,1,2)) + 1
             ELSE -1
        END
        ) BETWEEN 20 AND 29;
--==>> 조회 결과 없음
...
WHERE SUBSTR(SSN,8,1) IN ('1','3')
ORDER BY
    CASE WHEN SUBSTR(SSN,8,1) = '1' THEN EXTRACT(YEAR FROM SYSDATE) - (TO_NUMBER(SUBSTR(SSN,1,2)) + 1899)
         WHEN SUBSTR(SSN,8,1) = '3' THEN EXTRACT(YEAR FROM SYSDATE) - (TO_NUMBER(SUBSTR(SSN,1,2)) + 1999)
         ELSE -1
    END ;
--==>>
/*
...
SELECT NAME, CITY, RTRIM(BUSEO,'부') "부서", TEL
FROM TBL_INSA
...
SELECT CASE WHEN COMMISSION_PCT IS NOT NULL THEN '커미션받는사원'
            ...
            ELSE '알수없음'
       END "구분", EMPLOYEE_ID, COMMISSION_PCT
FROM EMPLOYEES
)T
...
SELECT T.연도, COUNT(*) " 직원수"
FROM
(
    SELECT (TO_NUMBER(SUBSTR(SSN,1,2)) + 1899) "연도"
    FROM TBL_INSA
) T
GROUP BY T.연도;
--==>>
...
FROM TBL_INSA
ORDER BY SUBSTR(SSN,3,2),  TO_NUMBER(SUBSTR(SSN,1,2))+ 1899 DESC;
--==>>
/*
이남신	810101-1010101
조미숙	790102-2777777
...
FROM
(
SELECT DECODE(SUBSTR(SSN,8,1),'1','남자','2','여자','3','남자','4','여자','성별확인불가') "성별"
FROM TBL_INSA
WHERE BUSEO = '영업부'
) T
GROUP BY T.성별;
...
SELECT BUSEO "부서"
    , SUM(CASE WHEN SUBSTR(SSN,8,1) IN ('1','3') THEN 1 ELSE 0 END) "남자인원수"
...
SELECT T.CITY, T.BUSEO
    , SUM(CASE WHEN T.성별 = '남자' THEN 1 ELSE 0 END) "남자인원수"
    , SUM(CASE WHEN T.성별 = '여자' THEN 1 ELSE 0 END) "여자인원수"
    , SUM(CASE WHEN T.성별 = '남자' THEN T.급여 ELSE 0 END) "남자급여합"
    , SUM(CASE WHEN T.성별 = '여자' THEN T.급여 ELSE 0 END) "여자급여합"
FROM
(
    SELECT BUSEO, CITY
...
--==>>
/*
서울	기획부	2	1	5420000	2070000
서울	개발부	0	2	      0	1994000
서울	홍보부	0	1	      0	1095000
서울	인사부	1	0	2465000	      0
서울	영업부	4	5	7422000	7062000
서울	자재부	0	1	      0	1150400
서울	총무부	2	1	4090000	1044000
*/
...
-- 51. 부서별 남,여 인원수 출력. 여자인원수가 5명 이상인 부서만 조회.SELECT *
...
FROM TBL_INSA
GROUP BY BUSEO
HAVING SUM(CASE WHEN SUBSTR(SSN,8,1) IN ('2','4') THEN 1 ELSE 0 END) >= 5 ;
...
SELECT NAME "이름"
    ...
    ,CASE
         WHEN SUBSTR(SSN,8,1) IN ('1','2') THEN EXTRACT(YEAR FROM SYSDATE) - (TO_NUMBER(SUBSTR(SSN,1,2)) + 1899)
...
SELECT T.월, COUNT(*)||'명' "인원수"
FROM
(
    SELECT CONCAT(LTRIM(TO_CHAR(IBSADATE, 'MM'),'0'),'월') "월"
        , IBSADATE
    FROM TBL_INSA
) T
GROUP BY T.월;
...
SELECT NAME "이름"
    , CASE  WHEN SUBSTR(SSN,8,1) IN ('1','2') THEN '19'||SUBSTR(SSN,1,2)||'-'|| SUBSTR(SSN,3,2)||'-'||SUBSTR(SSN,5,2)
            WHEN SUBSTR(SSN,8,1) IN ('3','4') THEN '20'||SUBSTR(SSN,1,2)||'-'|| SUBSTR(SSN,3,2)||'-'||SUBSTR(SSN,5,2)
      ELSE '생년월일알수없음'
      END "생년월일"
...
*/
 ...
 58. 근무년수가 10년 이상인 사람 조회. (이름, 입사일)  -- 연도차이라서 정확한 근무년수는 아님(add_months)
 
SELECT IBSADATE
FROM TBL_INSA
WHERE TO_CHAR(SYSDATE, 'YYYY-MM-DD') -TO_CHAR(IBSADATE, 'YYYY-MM-DD');

SELECT NAME, IBSADATE
FROM TBL_INSA
WHERE TRUNC(MONTHS_BETWEEN(SYSDATE, TO_DATE(IBSADATE, 'YYYY-MM-DD')) / 12) >= 10;
--==>>
/*
홍길동	1998-10-11
이순신	2000-11-29
...
*/
 ...
 60. 근무년수가 5~10년인 사람 조회. (이름, 입사일)  -- 58번과 비슷한 느낌 정확힌 군무기간이 아님 add_months
SELECT T.NAME, T.IBSADATE
FROM(
    SELECT NAME
         , IBSADATE
...
FROM TBL_INSA
GROUP BY BUSEO
HAVING SUM(CASE WHEN SUBSTR(SSN,8,1) IN ('1', '3') THEN 1 ELSE 0 END) >=5;
--==>>
/*
개발부	6
영업부	8
...
SELECT COUNT(*) "전체"
     , SUM(CASE WHEN EXTRACT(YEAR FROM IBSADATE) = 2000 THEN 1 ELSE 0 END) "2000"
     , SUM(CASE WHEN EXTRACT(YEAR FROM IBSADATE) = 1999 THEN 1 ELSE 0 END) "1999"
     , SUM(CASE WHEN EXTRACT(YEAR FROM IBSADATE) = 1998 THEN 1 ELSE 0 END) "1998"
FROM TBL_INSA;
--==>>
/*
60	12	10	6
...
SELECT COUNT(*) "전체"
     , SUM(CASE WHEN CITY = '서울' THEN 1 ELSE 0  END) "서울"
     , SUM(CASE WHEN CITY = '인천' THEN 1 ELSE 0  END) "인천"
     , SUM(CASE WHEN CITY = '경기' THEN 1 ELSE 0  END) "경기"
FROM TBL_INSA;
--==>>
/* 60	20	9	13*/
...
WHERE ((SELECT COUNT(*)
        FROM TBL_INSA
        WHERE BASICPAY > T.BASICPAY) < (SELECT COUNT(*) * 0.1
                                        FROM TBL_INSA));
--==>>
/*
홍길동	2610000
...
SELECT T1.NAME, T1.BUSEO, T1.BASICPAY
FROM TBL_INSA T1 JOIN (SELECT BUSEO, MAX(BASICPAY) "최대기본급"
                    FROM TBL_INSA
                    GROUP BY BUSEO) T2
ON T1.BUSEO = T2.BUSEO AND T1.BASICPAY = T2.최대기본급;
--==>>
/*
허경운	총무부	2650000
이순애	개발부	2550000
...
-- 85. 남, 여별 기본급 순위 조회.
SELECT T.NAME, T.성별
    ,   RANK() OVER (PARTITION BY T.성별 ORDER BY BASICPAY DESC) AS "순위"
FROM
(
    SELECT CASE WHEN SUBSTR(SSN,8,1) IN ('1','3')  THEN '남자'
                WHEN SUBSTR(SSN,8,1) IN ('2','4')  THEN '여자'
                ELSE '성별확인불가'
            END "성별", NAME, BASICPAY
    FROM TBL_INSA
) T;
--==>>
/*
...
SELECT *
FROM(
    SELECT CITY, (BASICPAY+SUDANG) "급여"
...
HAVING AVG(BASICPAY + SUDANG) = (
    ...
    FROM
    (
        SELECT AVG(BASICPAY + SUDANG) "평균급여"
        FROM TBL_INSA
        GROUP BY CITY
    ) T
);
...
SELECT T.BUSEO, T.여자인원수
FROM(
SELECT BUSEO, SUM(CASE WHEN SUBSTR(SSN,8,1) IN ('2','4') THEN 1 ELSE 0 END)  "여자인원수"
    , DENSE_RANK() OVER(ORDER BY SUM(CASE WHEN SUBSTR(SSN,8,1) IN ('2','4') THEN 1 ELSE 0 END) DESC) "순위"
FROM TBL_INSA
GROUP BY BUSEO
) T
WHERE T.순위 = 1;
...
SELECT NAME "이름", BUSEO "부서", CITY "출신도", BASICPAY "기본급", SUDANG "수당", (BASICPAY+SUDANG) "급여"
    ,  CASE WHEN (BASICPAY+SUDANG) >=2500000 THEN (BASICPAY+SUDANG)*0.02
            ...
            ELSE 0
        END  "세금"
    , (BASICPAY+SUDANG) - ( CASE WHEN (BASICPAY+SUDANG) >=2500000 THEN (BASICPAY+SUDANG)*0.02
                                WHEN (BASICPAY+SUDANG) >=2000000 THEN (BASICPAY+SUDANG)*0.01
                                ELSE 0
                            END ) "실수령액"
FROM TBL_INSA T;
...
SELECT BUSEO, AVG(BASICPAY+SUDANG) "평균급여"
     , CASE WHEN AVG(BASICPAY+SUDANG) >2000000  THEN 'A등급'
            ...
            WHEN AVG(BASICPAY+SUDANG) <1500000  THEN 'C등급'
           ELSE '등급외'
       END "등급"
FROM TBL_INSA
GROUP BY BUSEO;
--==>>
/*
총무부	1862285.71428571428571428571428571428571	B등급
개발부	1528785.71428571428571428571428571428571	B등급
영업부	1753762.5	B등급
기획부	2020714.28571428571428571428571428571429	A등급
인사부	1689350	B등급
자재부	1559400	B등급
홍보부	1579500	B등급
*/
